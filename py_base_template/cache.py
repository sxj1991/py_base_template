...
def write_data_to_cache(key, value, expire=1800):
    """
    将数据直接写入缓存
    :param key：唯一标识
    :param value：写入的数据
...
def refresh_cache(key, value, expire=1800):
    if exist_cache(key):
        write_data_to_cache(key, value, expire)
    else:
        raise Exception("缓存不存在")


def write_json_to_cache(key, value, expire=1800):
    """
    将数据转为json格式写入缓存
    :param key：唯一标识
    :param value：写入的数据
...
def write_counter_to_cache(key, step=1, expire=1800):
    """
    计数器写入缓存
    :param key：唯一标识
...
def base_caches():
    ...
    def _deco(func):
        def __deco(*args, **kwargs):
            ...
            cache_info = read_data_from_cache(base_key)
            if not cache_info:
                result = func(*args, **kwargs)
                write_data_to_cache(base_key, result)
            else:
                result = cache_info
            return result
